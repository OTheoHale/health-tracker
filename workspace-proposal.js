...
(function(root){
  ...
  function build(date){
    ...
    container('dw-workouts','Workouts',{category:'health-physical',workspaceKind:'workouts',anchor:'midday'});
    add('dw-cardio','Cardio',{parentId:'dw-workouts',category:'health-physical',workspaceKind:'cardio',optional:true,
      recurrence:weekly(4),
    ...
    add('dw-strength','Strength training',{parentId:'dw-workouts',category:'health-physical',workspaceKind:'strength',optional:true,recurrence:weekly(4),
    ...
    add('dw-fasting','Fasting',{category:'faith',workspaceKind:'fasting',anchor:'morning',remap:true,recurrence:{kind:'weekly',days:[3,5]},
      normal:target('Keep the fast (Wednesday and Friday); EOTC seasons are added separately once their dates are reviewed')});
    ...
    add('dw-sleep','Sleep 8 hours',{category:'health-mental',workspaceKind:'sleep',anchor:'morning',remap:true,matching:{kind:'sleep',minimum:480},
      normal:target('8 hours asleep in the night that ended this morning',480),minimum:target('No lower completion threshold')});
    ...
    add('dw-shower','Shower',{category:'hygiene',workspaceKind:'hygiene',remap:true,normal:target('Shower')});
    for(const number of [1,2])add('dw-bathroom-'+number,'Bathroom '+number,{category:'hygiene',workspaceKind:'bathroom',remap:true,normal:target('Reported bathroom visit '+number+'; requires its own event'),scoring:{importance:3,difficulty:3,eligible:true}});
    ...
    const notes=[
      ...
      'Preserve the existing Cardio ID and its history. Cardio now repeats weekly on any 4 days (Mintay, 2026-09-23); the earlier September 21–27 end date is not carried forward.',
    ...
    const policies=[
      ...
      'Workouts from any recording app count (Mintay, 2026-09-23); a type the rules cannot place is chosen once as cardio, strength or neither. Cardio combines qualifying workouts within a day, counting one session recorded by two apps once: 20 minutes qualifies one weekly day, 45 closes the daily target, 60 anchors 10 proposed base points. Steps and exercise-minute aggregates do not substitute.',
    ...
    const groups=[
      ...
      {id:'hygiene',name:'Hygiene',icon:'care',order:3.5},
